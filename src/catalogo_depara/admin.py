...
class CatalogoDeparaAdmin(admin.ModelAdmin):
    ...
    list_display = ["id_catalogo", "tabela_origem", "descricao", "ativo", "criado_em"]
    list_filter = ["ativo", "criado_em"]
    search_fields = ["tabela_origem", "descricao"]
    readonly_fields = ["id_catalogo", "criado_em", "atualizado_em"]

    fieldsets = (
        ("Informações Principais", {"fields": ("tabela_origem", "descricao", "ativo")}),
        (
            "Metadados",
            {"fields": ("id_catalogo", "criado_em", "atualizado_em"), "classes": ("collapse",)},
        ),
    )
