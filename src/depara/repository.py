...
class DeparaRepository:
    ...
    def get_all(filters: Optional[dict] = None) -> List[DePara]:
        ...
        if filters:
            ...
            if "codigo_origem" in filters:
                queryset = queryset.filter(codigo_origem__icontains=filters["codigo_origem"])

            if "codigo_destino" in filters:
                queryset = queryset.filter(codigo_destino__icontains=filters["codigo_destino"])
    ...
    def get_by_catalogo(id_catalogo: int) -> List[DePara]:
        ...
        """
        return list(
            DePara.objects.select_related("id_catalogo")
    ...
    @staticmethod
    def exists_mapping(
        id_catalogo: int, codigo_origem: str, codigo_destino: str, exclude_id: Optional[int] = None
    ) -> bool:
        """
        ...
        query = DePara.objects.filter(
            id_catalogo=id_catalogo, codigo_origem=codigo_origem, codigo_destino=codigo_destino
        )
