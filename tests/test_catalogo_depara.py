...
class CatalogoDeparaModelTest(TestCase):
    ...
    def setUp(self):
        self.catalog = CatalogoDePara.objects.create(
            tabela_origem="teste_table", descricao="Test Description", ativo=True
        )
    ...
    def test_catalog_ordering(self):
        CatalogoDePara.objects.create(
            tabela_origem="another_table", descricao="Another Description"
        )
...
class CatalogoDeparaServiceTest(TestCase):
    ...
    def test_create_catalog_success(self):
        data = {"tabela_origem": "test_table", "descricao": "Test catalog", "ativo": True}
    ...
    def test_create_catalog_missing_required_field(self):
        data = {"descricao": "Test catalog"}

        with self.assertRaises(ValueError):
            self.service.create_catalogo(data)

    def test_list_catalogs(self):
        CatalogoDePara.objects.create(tabela_origem="table1", descricao="Desc 1")
        CatalogoDePara.objects.create(tabela_origem="table2", descricao="Desc 2")
    ...
    def test_get_catalog_by_id(self):
        ...
        catalog = CatalogoDePara.objects.create(tabela_origem="test_table", descricao="Test")
    ...
    def test_update_catalog(self):
        catalog = CatalogoDePara.objects.create(
            tabela_origem="old_table", descricao="Old description"
        )

        updated = self.service.update_catalogo(
            catalog.id_catalogo, {"descricao": "New description"}
        )
    ...
    def test_activate_deactivate_catalog(self):
        catalog = CatalogoDePara.objects.create(tabela_origem="test_table", ativo=True)
